...
namespace UI.Controls
{
    ...
    /// </remarks>
    public partial class MaterialListViewBindable : MaterialListView
    {
        /// <summary>
        ...
        private ListViewItem BuildItemForRow(object row)
        {
            ...
            {
                case Customer c:
                    string[] rowText = new string[] { c.ID.ToString(), $"{c.Firstname} {c.Lastname}", c.Email };
                    return new ListViewItem(rowText);
                case OrderProduct opm:
                    rowText = new string[] { opm.Name, opm.Model, opm.Quantity.ToString(), $"${opm.Price.ToString("#.##")}", $"${opm.Total.ToString("#.##")}" };
                    return new ListViewItem(rowText) { Tag = opm };
                case Order om:
                    rowText = new string[] {
                        om.ID.ToString(),
                        om.Customer,
                        om.OrderStatus,
                        $"${om.Total.ToString("#.##")}",
                        om.DateAdded.ToShortDateString(),
                        om.DateModified.ToShortDateString()
                    };
                    return new ListViewItem(rowText);
                case Discount d:
                    rowText = new string[] {
                        //CommonDataProxy.Instance.CustomerGroups.Find(cid => cid.ID == pdm.Customer_group_id)?.Name,
                        d.CustomerGroupID.ToString(),
                        d.Quantity.ToString(),
                        d.Priority.ToString(),
                        $"${d.Price.ToString("#.##")}",
                        d.DateStart.ToShortDateString(),
                        d.DateEnd.ToShortDateString()
                    };
                    return new ListViewItem(rowText);
                case SpecialOffer so:
                    rowText = new string[] {
                        //CommonDataProxy.Instance.CustomerGroups.Find(cid => cid.ID == psm.CustomerGroupID)?.Name,
                        so.CustomerGroupID.ToString(),
                        so.Priority.ToString(),
                        $"${so.Price.ToString("#.##")}",
                        so.DateStart.ToShortDateString(),
                        so.DateEnd.ToShortDateString()
                    };
                    return new ListViewItem(rowText);
                case OrderTotal ot:
                    rowText = new string[] {
            ...
            switch (e.ListChangedType)
            {
                ...
                case ListChangedType.ItemAdded:
                    ...
                    if (drv == null || !drv.IsNew)
                    {
                        ...
                        BeginUpdate();
                        //Items.Insert(e.NewIndex, BuildItemForRow(newRow));                    // Commented: 20/Nov/2018 by Enrique Pérez
                        EndUpdate();
                    }
                    break;
                ...
                case ListChangedType.ItemDeleted:
                    //if (e.NewIndex < Items.Count)                                             // Commented: 2/Dec/2018 by Enrique Pérez
                    //{
        ...
        // COMMENTED ***************************************************************************************************************
        // private void CurrencyManager_PositionChanged(object sender, EventArgs e) => SetSelectedIndex(m_currencyManager.Position);
        ...
        private void CurrencyManager_ItemChanged(object sender, ItemChangedEventArgs e)
        {
            ...
            if (e.Index == -1)
            {
                ...
                if (!ReferenceEquals(m_bindingList, m_currencyManager.List))
                {
                    m_bindingList.ListChanged -=
                        new ListChangedEventHandler(BindingList_ListChanged);
